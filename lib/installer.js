...
function acquireQshell(version) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let fileName = `qshell-${osPlat == 'win32' ? 'windows' : osPlat}-${osArch}-v${version}${osPlat == 'win32' ? '.exe' : ''}`;
        ...
        const unzipFiles = yield fs_1.promises.readdir(extPath);
        console.log(`unzipFiles: ${unzipFiles.join('\n')}`);
        const cacheRst = yield tc.cacheFile(oldPath, `qshell${osPlat == 'win32' ? '.exe' : ''}`, 'qshell', version);
